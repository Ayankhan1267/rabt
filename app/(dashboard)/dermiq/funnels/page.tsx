...
const FUNNEL_STEPS = [
  { step: 1, name: 'Website Visitors', count: 12480, pct: 100, drop: null, widthPct: 100, health: 'good' },
  { step: 2, name: 'Skin Analysis Started', count: 4992, pct: 40, drop: 60, widthPct: 80, health: 'warn' },
  { step: 3, name: 'Analysis Completed', count: 3744, pct: 30, drop: 25, widthPct: 65, health: 'good' },
  { step: 4, name: 'Consultation Booked', count: 1122, pct: 9, drop: 70, widthPct: 48, health: 'bad' },
  { step: 5, name: 'Product Recommended', count: 2808, pct: 22.5, drop: null, widthPct: 56, health: 'good' },
  { step: 6, name: 'Added to Cart', count: 1872, pct: 15, drop: 33, widthPct: 42, health: 'warn' },
  { step: 7, name: 'Checkout Started', count: 1248, pct: 10, drop: 33, widthPct: 30, health: 'warn' },
  { step: 8, name: 'Purchase Completed', count: 874, pct: 7, drop: 30, widthPct: 20, health: 'bad' },
]

const DROPOFFS = [
  {
    from: 'Analysis', to: 'Consultation', drop: 78, fix: 'Add consultation CTA in report page',
    impact: '+2.4% conversion', link: '/dermiq/consultations', urgency: 'high',
  },
  {
    from: 'Cart', to: 'Checkout', drop: 33, fix: 'WhatsApp reminder at 2hrs after cart abandonment',
    impact: '+1.8% conversion', link: '/dermiq/marketing', urgency: 'medium',
  },
  {
    from: 'Checkout', to: 'Purchase', drop: 30, fix: 'Add UPI deeplink for faster payment',
    impact: '+1.2% conversion', link: '/dermiq/finance', urgency: 'medium',
  },
]

const CHANNELS = [
  { channel: 'Organic Search', visitors: 4280, conversions: 312, revenue: 202488, cost: 0 },
  { channel: 'WhatsApp', visitors: 2840, conversions: 248, revenue: 161052, cost: 4200 },
  { channel: 'Instagram', visitors: 1920, conversions: 118, revenue: 76582, cost: 18000 },
  { channel: 'Direct', visitors: 2180, conversions: 142, revenue: 92258, cost: 0 },
  { channel: 'Email', visitors: 860, conversions: 54, revenue: 35046, cost: 2100 },
]

const AB_TESTS_INITIAL = [
  { test: 'CTA Button Color', varA: '"Book Now" (teal)', varB: '"Book Now" (gold)', split: '50/50', winner: 'Variant B (+12% CTR)', status: 'complete' },
  { test: 'Analysis Page Hero', varA: 'Text only', varB: 'Text + image', split: '50/50', winner: 'Running (Day 4/14)', status: 'running' },
  { test: 'Consultation Price', varA: '₹499', varB: '₹399', split: '50/50', winner: 'Variant B (+8% bookings)', status: 'complete' },
]
    ...
    <div style={{ fontFamily: 'DM Sans, sans-serif', color: C.dark }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
          ...
          <div>
            <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: 24, fontWeight: 700, color: C.dark, margin: 0 }}>Conversion Funnel Tracker</h1>
            <p style={{ fontSize: 12, color: C.mu, margin: 0 }}>End-to-end user journey · Drop-off analysis · Channel attribution</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 16, padding: 28, marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginTop: 24 }}>
          {[
            { label: 'Overall Conversion', value: '7.0%', color: C.teal },
            { label: 'Avg Drop Per Step', value: '43%', color: C.orange },
            { label: 'Revenue per Visitor', value: '₹66', color: C.green },
          ].map((k, i) => (
          ...
          {DROPOFFS.map((d, i) => (
            <div key={i} style={{ background: '#fff', border: `1px solid ${d.urgency === 'high' ? C.red : C.orange}30`, borderRadius: 14, padding: 20, borderLeft: `4px solid ${d.urgency === 'high' ? C.red : C.orange}` }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
                ...
                <button onClick={() => toast.success(`Navigating to fix for ${d.from} → ${d.to}`)} style={{ padding: '10px 18px', background: d.urgency === 'high' ? C.red : C.orange, color: '#fff', border: 'none', borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'DM Sans, sans-serif' }}>
                  Fix This →
                </button>
              </div>
            </div>
                  ...
                  <tr key={i} style={{ borderTop: `1px solid ${C.border}` }}>
                    <td style={{ padding: '12px 16px', fontSize: 13, fontWeight: 700 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ width: 8, height: 8, borderRadius: '50%', background: [C.green, C.teal, C.purple, C.blue, C.gold][i] }} />
                        {ch.channel}
                      </div>
                    </td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: 20, padding: 32, width: 460, maxWidth: '90vw', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input value={newTest.name} onChange={e => setNewTest(n => ({ ...n, name: e.target.value }))} placeholder="e.g. Homepage Banner" style={{ width: '100%', padding: '10px 12px', border: `1px solid ${C.border}`, borderRadius: 10, fontSize: 13, fontFamily: 'DM Sans, sans-serif' }} />
            </div>
            <div style={{ marginBottom: 14 }}>
              ...
              <input value={newTest.varA} onChange={e => setNewTest(n => ({ ...n, varA: e.target.value }))} placeholder="e.g. Original design" style={{ width: '100%', padding: '10px 12px', border: `1px solid ${C.border}`, borderRadius: 10, fontSize: 13, fontFamily: 'DM Sans, sans-serif' }} />
            </div>
            <div style={{ marginBottom: 22 }}>
              ...
              <input value={newTest.varB} onChange={e => setNewTest(n => ({ ...n, varB: e.target.value }))} placeholder="e.g. New design with hero image" style={{ width: '100%', padding: '10px 12px', border: `1px solid ${C.border}`, borderRadius: 10, fontSize: 13, fontFamily: 'DM Sans, sans-serif' }} />
            </div>
