...
const TABS = [
  { id: 'hero',         l: 'ðŸ  Hero',          desc: 'Main banner content' },
  { id: 'testimonials', l: 'â­ Testimonials',   desc: 'Customer reviews' },
  { id: 'trust',        l: 'ðŸ† Trust Badges',  desc: 'Stats & trust signals' },
  { id: 'seo',          l: 'ðŸ” SEO',           desc: 'Meta tags & AI SEO' },
  { id: 'analytics',    l: 'ðŸ“Š Analytics',     desc: 'Traffic & performance' },
  { id: 'settings',     l: 'âš™ï¸ Settings',      desc: 'Links & general' },
]

const DEFAULT_CONTENT = {
  hero_badge: "India's First Pulses & Grains Skincare",
  hero_title: "Where Nature Bonds With Skin.",
  hero_subtitle: '"Rabt" â€” the timeless bond between skin and nature.',
  ...
  trust_stat4_num: '4.9â˜…',
  trust_stat4_label: 'Average Rating',
}

const DEFAULT_TESTIMONIALS = [
  { name: 'Komal Singh', skin: 'Acne-Prone Â· Verified Buyer', quote: 'My skin is smoother and clearer than ever. I finally feel confident going makeup-free!', product: 'ðŸ§´ Niacinamide Serum', image: 'https://rabtnaturals.com/testi2.jpg', stars: 5 },
  { name: 'Mukul Parmar', skin: 'Dry Hair Â· Verified Buyer', quote: 'Soft, shiny, and smells amazing! Highly recommend for anyone with damaged hair.', product: 'ðŸ’§ Hydrating Serum', image: 'https://rabtnaturals.com/testi1.jpg', stars: 5 },
  { name: 'Somya Jain', skin: 'Combination Â· Verified Buyer', quote: 'Perfect coverage without feeling heavy. Lasts all day even in humid weather!', product: 'âœ¨ Vitamin C Cream', image: 'https://rabtnaturals.com/testi3.jpg', stars: 5 },
  { name: 'Priya Sharma', skin: 'Oily Skin Â· Verified Buyer', quote: 'Oil control is incredible. My T-zone stays matte all day without drying out.', product: 'ðŸŒ¿ Oats Moisturizer', image: 'https://rabtnaturals.com/testi2.jpg', stars: 5 },
  { name: 'Rahul Verma', skin: 'Sensitive Â· Verified Buyer', quote: 'Zero irritation. My skin barrier feels stronger than it has in years.', product: 'â˜€ï¸ SPF 50+ Sunscreen', image: 'https://rabtnaturals.com/testi1.jpg', stars: 5 },
  { name: 'Anjali Mehta', skin: 'Dark Spots Â· Verified Buyer', quote: 'My dark spots faded within a month. The AI quiz recommended exactly what I needed.', product: 'ðŸ”¬ Alpha Arbutin Serum', image: 'https://rabtnaturals.com/testi3.jpg', stars: 5 },
]
...
export default function WebsiteManagerPage() {
  ...
  async function saveContent() {
    ...
    toast.success('Content saved! âœ… Landing page update ho jayega.')
    setSaving(false)
  }
  ...
  async function saveTestimonials() {
    ...
    toast.success('Testimonials saved! âœ…')
    setSaving(false)
  }
  ...
  async function addTestimonial() {
    const newT = { name: 'New Customer', skin: 'Skin Type Â· Verified Buyer', quote: 'Amazing product!', product: 'ðŸŒ¿ Product Name', image: '', stars: 5 }
  ...
  async function uploadVideo(idx: number, file: File) {
    ...
    try {
      ...
      setTestimonials(updated)
      await supabase.from('app_settings').upsert({ key: 'landing_testimonials', value: JSON.stringify(updated) })
      toast.success('Video uploaded! âœ…', { id: 'upload' })
    } catch (e: any) {
  ...
  async function uploadImage(idx: number, file: File) {
    ...
    try {
      ...
      setTestimonials(updated)
      await supabase.from('app_settings').upsert({ key: 'landing_testimonials', value: JSON.stringify(updated) })
      toast.success('Image uploaded! âœ…', { id: 'img-upload' })
    } catch (e: any) {
...
No markdown, no explanation, just JSON.`
      ...
      toast.success('AI SEO generated! âœ…', { id: 'ai-seo' })
    } catch {
      toast.error('AI error â€” manually fill karo', { id: 'ai-seo' })
    }
    setAiLoading(false)
  }
...
Return ONLY the text, no explanation, no quotes.`
      ...
      toast.success('Content generated! âœ…', { id: 'ai-content' })
    } catch {
    ...
    <div>
      ...
      {tab === 'hero' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 16 }}>
          <div className="card">
            <div style={{ fontFamily: 'Syne', fontSize: 14, fontWeight: 800, marginBottom: 16 }}>ðŸ  Hero Section</div>

            {[
              { key: 'hero_badge',        l: 'Badge Text',      ph: "India's First..." },
              { key: 'hero_title',        l: 'Main Heading',    ph: 'Where Nature...' },
              { key: 'hero_subtitle',     l: 'Italic Subtitle', ph: '"Rabt" â€” ...' },
              { key: 'hero_cta_primary',  l: 'Primary Button',  ph: 'Know Your Skin â†’' },
              { key: 'hero_cta_secondary',l: 'Secondary Button',ph: 'Explore Products' },
              { key: 'hero_note',         l: 'Small Note',      ph: 'Free Â· 2 Minutes...' },
              ...
              <div key={f.key}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                  ...
                  <button onClick={() => generateAIContent(f.key)} disabled={aiLoading} style={{ fontSize: 10, padding: '2px 8px', background: 'rgba(0,151,167,0.1)', border: 'none', borderRadius: 4, color: 'var(--teal)', cursor: 'pointer', fontWeight: 600 }}>âœ¨ AI</button>
                </div>
          ...
          <div className="card" style={{ background: 'linear-gradient(135deg,#0A1414,#1A2828)', color: '#fff' }}>
            ...
            <div style={{ fontSize: 10, color: '#4A6464', marginTop: 10 }}>âœ¦ {content.hero_note}</div>
          </div>
        </div>
      )}
      ...
      {tab === 'testimonials' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ fontFamily: 'Syne', fontSize: 14, fontWeight: 800 }}>â­ Customer Testimonials ({testimonials.length})</div>
                ...
                {editingTesti === i ? (
                  <div>
                    ...
                    {[
                      ...
                      { k: 'skin',    l: 'Skin Type',ph: 'Oily Â· Verified Buyer' },
                      { k: 'quote',   l: 'Quote',   ph: 'Amazing product...' },
                      { k: 'product', l: 'Product', ph: 'ðŸŒ¿ Product Name' },
                    ].map(f => (
                      <div key={f.k}>
                    ...
                    <div style={{ marginBottom: 12 }}>
                      ...
                      {t.video_url && <div style={{ fontSize: 10, color: 'var(--green)', marginTop: 4 }}>âœ“ Video uploaded</div>}
                    </div>
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                      ...
                      <div style={{ color: 'var(--gold)', fontSize: 12 }}>{'â˜…'.repeat(t.stars || 5)}</div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => setEditingTesti(i)} style={{ flex: 1, padding: '6px', background: 'var(--blL)', border: 'none', borderRadius: 6, color: 'var(--blue)', fontWeight: 600, fontSize: 11, cursor: 'pointer' }}>âœï¸ Edit</button>
                      <button onClick={() => deleteTestimonial(i)} style={{ padding: '6px 10px', background: 'var(--rdL)', border: 'none', borderRadius: 6, color: 'var(--red)', fontSize: 11, cursor: 'pointer' }}>ðŸ—‘ï¸</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      {tab === 'trust' && (
        <div className="card">
          <div style={{ fontFamily: 'Syne', fontSize: 14, fontWeight: 800, marginBottom: 16 }}>ðŸ† Trust Badges & Stats</div>
      ...
      {tab === 'seo' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div style={{ fontFamily: 'Syne', fontSize: 14, fontWeight: 800 }}>ðŸ” SEO Settings</div>
              <button onClick={generateAISEO} disabled={aiLoading} style={{ padding: '7px 14px', background: 'linear-gradient(135deg,#8B5CF6,#6D28D9)', border: 'none', borderRadius: 8, color: '#fff', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                {aiLoading ? 'â³ Generating...' : 'âœ¨ AI Generate SEO'}
              </button>
            </div>

            {[
              { key: 'meta_title',    l: 'Page Title',         ph: 'Rabt Naturals â€“ ...', max: 60 },
            ...
            <div className="card">
              ...
              {[
                ...
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: '1px solid var(--b1)' }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: item.ok ? 'var(--green)' : 'var(--red)' }}>{item.ok ? 'âœ“ Good' : 'âœ— Fix'}</span>
                </div>
              ))}
      ...
      {tab === 'analytics' && (
        <div>
          <div style={{ background: 'rgba(0,151,167,0.06)', border: '1px solid rgba(0,151,167,0.2)', borderRadius: 10, padding: '14px 16px', marginBottom: 20 }}>
            <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 6 }}>ðŸ“Š Analytics Setup</div>
            <div style={{ fontSize: 12, color: 'var(--mu2)', lineHeight: 1.7 }}>
              ...
              Ya Vercel Analytics use karo â€” automatically traffic track karta hai.
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: 12, marginBottom: 20 }}>
            {[
              { l: 'Total Visits',    v: analytics?.total_visits || 'â€”',    icon: 'ðŸ‘ï¸', color: 'var(--teal)' },
              { l: 'Today',          v: analytics?.today_visits || 'â€”',    icon: 'ðŸ“…', color: 'var(--blue)' },
              { l: 'Bounce Rate',    v: analytics?.bounce_rate || 'â€”',     icon: 'â†©ï¸', color: 'var(--orange)' },
              { l: 'Avg Load Time',  v: analytics?.load_time || 'â€”',       icon: 'âš¡', color: 'var(--green)' },
              { l: 'Conversions',    v: analytics?.conversions || 'â€”',     icon: 'ðŸŽ¯', color: 'var(--gold)' },
              { l: 'Mobile Users',   v: analytics?.mobile_pct || 'â€”',      icon: 'ðŸ“±', color: 'var(--purple)' },
            ].map((s, i) => (
      ...
      {tab === 'settings' && (
        <div className="card">
          <div style={{ fontFamily: 'Syne', fontSize: 14, fontWeight: 800, marginBottom: 16 }}>âš™ï¸ General Settings</div>
