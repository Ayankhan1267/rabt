...
export default function AdminPage() {
  ...
  async function addProfile() {
    ...
    await supabase.from('profiles').insert({ name: form.name, email: form.email, role: form.role, phone: form.phone })
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: 'var(--mu)', fontSize: 12.5, marginTop: 4 }}>{profiles.length} team members · Manage roles, leads & tasks</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {leads.slice(0, 20).map(l => (
            <div key={l.id} style={{ background: 'var(--s1)', border: '1px solid var(--b1)', borderRadius: 12, padding: '14px 16px' }}>
              ...
              <select value={l.assigned_to?.id || ''} onChange={e => assignLead(l.id, e.target.value)} style={{ width: '100%', background: 'var(--s2)', border: '1px solid var(--b2)', borderRadius: 8, padding: '7px 10px', color: 'var(--tx)', fontSize: 12, fontFamily: 'Outfit', outline: 'none', cursor: 'pointer', marginBottom: 6 }}>
                <option value="">— Unassigned —</option>
                {profiles.map(p => <option key={p.id} value={p.id}>{p.name} ({p.role})</option>)}
              </select>
              {l.assigned_to?.name && <div style={{ fontSize: 11, color: 'var(--green)' }}>✅ {l.assigned_to.name}</div>}
            </div>
          ))}
          ...
          {tasks.slice(0, 20).map(t => {
            ...
            return (
              <div key={t.id} style={{ background: 'var(--s1)', border: '1px solid var(--b1)', borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, flex: 1, marginRight: 8 }}>{t.title}</div>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: pc[t.priority] || 'var(--mu)', flexShrink: 0 }} />
                </div>
                <div style={{ fontSize: 12, color: 'var(--mu)', marginBottom: 10 }}>{t.priority} · Due {t.due_date || '—'}</div>
                <select value={t.assigned_to?.id || ''} onChange={e => assignTask(t.id, e.target.value)} style={{ width: '100%', background: 'var(--s2)', border: '1px solid var(--b2)', borderRadius: 8, padding: '7px 10px', color: 'var(--tx)', fontSize: 12, fontFamily: 'Outfit', outline: 'none', cursor: 'pointer', marginBottom: 6 }}>
                  <option value="">— Unassigned —</option>
                  {profiles.map(p => <option key={p.id} value={p.id}>{p.name} ({p.role})</option>)}
                </select>
                {t.assigned_to?.name && <div style={{ fontSize: 11, color: 'var(--green)' }}>✅ {t.assigned_to.name}</div>}
              </div>
            )
          })}
      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)', zIndex: 5000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--s1)', border: '1px solid var(--b2)', borderRadius: 16, padding: '26px 30px', width: 440, maxWidth: '94vw' }}>
            <div style={{ fontFamily: 'Syne', fontSize: 17, fontWeight: 800, marginBottom: 20 }}>➕ Add Team Member</div>
            {[
            ...
            <div style={{ display: 'flex', gap: 9 }}>
              ...
              <button onClick={addProfile} style={{ flex: 1, padding: 10, background: 'linear-gradient(135deg,#D4A853,#B87C30)', border: 'none', borderRadius: 8, color: '#08090C', fontWeight: 700, fontSize: 12.5, cursor: 'pointer', fontFamily: 'Outfit' }}>Add Member</button>
            </div>
          </div>
        </div>
      )}
